...
export default async function TeacherRequestsPage() {
  ...
  if (teacherRequests.length > 0 && adminClient) {
      ...
      userResults
        ...
        .map((result) => [
          result.id,
          {
            ...
            full_name:
              (result.user?.user_metadata?.name as string | undefined) ?? null,
          },
        ]),
    );
    ...
    <main className="min-h-screen bg-slate-950 pt-28 pb-24 px-4">
      <div className="mx-auto w-full max-w-5xl space-y-8">
        <header className="space-y-1">
          <h1 className="text-3xl font-bold text-white">Teacher Requests</h1>
